...
angular.module('app.controllers', ['pascalprecht.translate', 'ngCookies'])
  ...
  //DonHangCtrl
  .controller('DonHangCtrl', ['$rootScope','$scope', 'danh_sach_don_hang', 'DataService', '$state', '$anchorScroll', '$timeout', 'scrollTo', function($rootScope, $scope, danh_sach_don_hang, DataService, $state, $anchorScroll, $timeout, scrollTo) {

    $scope.danh_sach_don_hang = danh_sach_don_hang || new DataService.init();
    ...
    $scope.refresh = function(){
      ...
      return DataService.get($scope.danh_sach_don_hang.id)
      .then(function(data){
        $scope.danh_sach_don_hang = data;
        $timeout(function(){
          $('table[ui-jq="footable"]').trigger('footable_redraw');
    ...
    $scope.create = function(){
      $rootScope.showLoader();
      DataService.create({
        ho_ten: $scope.danh_sach_don_hang.ho_va_ten,
        dia_chi_sdt: $scope.danh_sach_don_hang.dia_chi_sdt
      }).then(function(id){
        $rootScope.hideLoader();
        $scope.danh_sach_don_hang.id = id;
      })
    }
    ...
    $scope.update = function(){
      $rootScope.showLoader();
      DataService.edit({
        id: $scope.danh_sach_don_hang.id,
        ho_ten: $scope.danh_sach_don_hang.ho_va_ten,
        dia_chi_sdt: $scope.danh_sach_don_hang.dia_chi_sdt
      }).then(function(){
        $rootScope.hideLoader();
    ...
    $scope.apply_add = function(){
      $rootScope.showLoader();
      $scope.don_hang.id_giao_dich = $scope.danh_sach_don_hang.id;
      var don_hang = $scope.don_hang;
      DataService.ListCreate(don_hang)
      .then(function(){
    ...
    $scope.edit = function(index){
      $scope.don_hang = JSON.parse(JSON.stringify($scope.danh_sach_don_hang.list[index]));
      openForm();
    };
    ...
    $scope.apply_edit = function(){
      ...
      var don_hang = $scope.don_hang;
      $scope.don_hang = null;
      DataService.ListEdit(don_hang)
      .then(function(data){
        return $scope.refresh();
      }, function(){
        $scope.don_hang = don_hang;
      });
    };
    ...
    $scope.remove = function(id){
      $rootScope.showLoader();
      DataService.ListDelete(id).then(function(){
        return $scope.refresh();
      })
    }

    $scope.submit = function(){
      DataService.submit($scope.danh_sach_don_hang)
      .then(function(){
        $state.go('app.formAList');
      });
    };
  ...
  .controller('FormAListCtrl', ['$rootScope', '$scope', 'danh_sach_giao_dich', 'DataService', '$timeout', function($rootScope, $scope, danh_sach_giao_dich, DataService, $timeout) {
    $scope.danh_sach_giao_dich = danh_sach_giao_dich;

    $scope.refresh = function(){
      $rootScope.showLoader();
      return DataService.fetch()
      .then(function(data){
        $scope.danh_sach_giao_dich = data;
        $timeout(function(){
          $('table[ui-jq="footable"]').data('footable-sort').doSort(0);
          $rootScope.hideLoader();
        });
      })
    }
  ...
  .controller('FormECtrl', ['$scope','formEData',function($scope,formEData){
    $scope.data = formEData;
    var _showed = -1;
    $scope.showed = function(index){
      return _showed == index;
    }
    $scope.show = function(index){
      if (!$scope.showed(index)) _showed = index;
      else _showed = -1;
    }
  }])
  ;